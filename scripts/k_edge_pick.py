    ...
    for nd in anc:
        if isinstance(nd, Attn) and nd.query_matrix.shape[0] >= 1:
            if abs(float(nd.query_matrix[0, 0].item()) - MATCH_GAIN_LONG) < 1.0:
                cands.append(nd)
    print(f"[edge-pick] recency-pick Attn candidates (match_gain={MATCH_GAIN_LONG}): "
          f"{len(cands)}")
    for nd in cands:
        vin = nd.inputs[2]
        print(f"  id={nd.node_id} d_qk={nd.d_qk} d_v={nd.d_v} "
...
def _inspect(attn, cache, n, MATCH_GAIN_LONG) -> None:
    ...
    print(f"[edge-pick] query at pos {POS}: entity_u(q)={q:.3f}  tree_depth={q_depth}  "
    ...
    print(f"[edge-pick] edges matching (child~={q:.0f}, depth=={q_depth}): "
    ...
    for w, i in zip(topk.values.tolist(), topk.indices.tolist()):
        print(f"    pos {i:5d}  w={w:.6f}  logit={logits[i].item():.4f}  "
    ...
    if matches:
        ...
        for i, child, depth in matches[-6:]:
            print(f"    pos {i:5d}  w={softmax[i].item():.6f}  logit={logits[i].item():.4f}  "
                  f"content={content[i].item():.4f}  recency={recency[i].item():.4f}  "
    ...
    print(f"[edge-pick]   nearest int parent = {round(parent_recovered)}  "
    ...
    print(f"[edge-pick]   float32 ULP at that scale ~= {ulp:.4g}  vs recency gap "
    ...
    for label, qvec in (("current [2q,1,1]", qv32.clone()),
        ...
        print(f"  {label:30s}: winner pos {win}  parent={parent_win:.3f}  "
    ...
    for pos in (2060, 2291):
        kin = key_in[pos]
        print(f"  pos {pos}: child={kin[0].item():.5f}  -child^2={kin[1].item():.5f}  "
